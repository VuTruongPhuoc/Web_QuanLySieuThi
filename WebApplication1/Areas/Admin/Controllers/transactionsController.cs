...
namespace WebApplication1.Areas.Admin.Controllers
{
    public class transactionsController : Controller
    {
        ...
        public ActionResult Index(string id)
        {
            if (id == null)
            {
                return View(db.transactions.ToList().OrderByDescending(s => s.created));
            }else if(id == "000")// Chưa xác nhận
            {
                return View(db.transactions.Where(s=>s.status == 0).ToList().OrderByDescending(s => s.created));
            }
            else if (id == "001")//Đã xác nhận- chờ vận chuyển
            {
                return View(db.transactions.Where(s => s.status == 1).ToList().OrderByDescending(s => s.created));
            }
            else if (id == "002")//Giao thành công
            {
                return View(db.transactions.Where(s => s.status == 2).ToList().OrderByDescending(s => s.created));
            }
            else if (id == "010")// Hủy
            {
                return View(db.transactions.Where(s => s.status == -1).ToList().OrderByDescending(s => s.created));
            }
            else if (id == "020")// Giao thất bại
            {
                return View(db.transactions.Where(s => s.status == -2).ToList().OrderByDescending(s => s.created));
            }
            else
