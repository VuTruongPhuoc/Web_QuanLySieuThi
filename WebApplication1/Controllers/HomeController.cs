...
namespace WebApplication1.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            ViewBag.sanphammoinhat = db.products.OrderByDescending(s => s.created).Take(8).ToList();
            ViewBag.giamgiathapnhat = db.products.OrderByDescending(s => s.discount).Take(4).ToList();
        ...
        [HttpPost]
        public JsonResult XLLoginAdmin(string username, string password)
        {
            if (username != "" || password != "")
            {
                var useradmin = db.admins.Where(s => s.username == username && s.password == password).FirstOrDefault();
                if (useradmin != null)
                {
                    ViewBag.user_admin = useradmin;
                    return Json(new { code = 200, url = Url.Action("Index", "admins", new { area = "Admin" }), msg = "Thành công", _user = useradmin }, JsonRequestBehavior.AllowGet);
                }
                else
                {
            ...
            {
                return Json(new { code = 500, msg = "Vui lòng nhập đủ thông tin" }, JsonRequestBehavior.AllowGet);
            }
        }
        [HttpGet]
        ...
        [HttpPost]
        public JsonResult XLRegister(string gmail, string password, string username, string address, string phone)
        {
            try
            {
                using (JP_QuanLySieuThiEntities1 db = new JP_QuanLySieuThiEntities1())
                {
                    // Kiểm tra xem email đã được sử dụng chưa
                    if (db.users.Any(u => u.gmail == gmail))
                    {
                        return Json(new { code = 400, msg = "Email đã được sử dụng. Vui lòng chọn một email khác." });
                    }
                    ...
                    return Json(new { code = 200, msg = "Đăng ký tài khoản thành công!" });
                }
            }
            catch (Exception ex)
            {
                return Json(new { code = 500, msg = "Đã xảy ra lỗi trong quá trình đăng ký. Vui lòng thử lại sau." });
            }
        }
        ...
        //Sản phẩm cùng loại
        public ActionResult Product_View_Transaction(int? page,string id, string check, int? check_search)
        {
            if (page == null) page = 1;
            int pageSize = 18;
            ...
            System.Diagnostics.Debug.WriteLine("id tìm kiếm: " + id_search + "- Check: " + id_check + "- check_search:" + check_search);
            if (id_check == null)
            {
                if (id_search == null)
                {
                ...
                else if(check == null && check_search == null)
                {
            ...
            try
            {
                ...
                if (id!= null || soluong!= null)
                {
                    ...
                    if (product_item != null)
                    {
                        ...
                        {
                            return Json(new { code = 400, msg = "Sản phẩm đã hết hàng" }, JsonRequestBehavior.AllowGet);
                        }    
        ...
        {
            //bool result = SendEmail("phuoc@gmail.com", "Test guibgmail", "Xin chao toi la phuoc");
        ...
        public JsonResult Xuly_Transaction(string checkgiaohang, string note, string hinhthucgiaohang)
        {
            try
            {
                ...
                return Json(new { code = 200,msg = _msg, _lis_transactions = lis_transactions,_list_order = order_Temps }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                return Json(new { code = 500, msg = "Mua lỗi" + ex }, JsonRequestBehavior.AllowGet);
